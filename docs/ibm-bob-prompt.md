...
> Inspect this Java 25 / Quarkus 3.39.3 Maven reactor. Add a read-only `get_change_summary` MCP tool that returns only the `deployment` and `change` fields for an existing incident, using a parameterized query. Extend the gateway's read-tool allowlist and strict incident ID validation, and update the investigator instructions to use the new tool when helpful. Preserve the private backend credential, human approval checks and follow-up idempotency. Do not expose arbitrary SQL or change infrastructure. Add tests proving authorized reads succeed and write escalation still fails. Update README. Show me the planned files and diff before applying the change. Never include credentials in source or output.
