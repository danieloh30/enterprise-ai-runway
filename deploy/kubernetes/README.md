...
4. `runway-runtime` additionally needs `DEMO_API_KEY` (a disabled/unusable placeholder if OIDC is enabled), `GATEWAY_READ_KEY`, `GATEWAY_WRITE_KEY`, `MCP_GATEWAY_URL=http://runway-gateway:8091`, `OPENAI_API_KEY` from a Secret (or `LLM_API_KEY` for another compatible provider), optional `LLM_BASE_URL` and `LLM_MODEL`, `OIDC_ENABLED=true`, `OIDC_AUTH_SERVER_URL`, `OIDC_CLIENT_ID`. Set `GATEWAY_KIND` to the actual gateway arrangement.
